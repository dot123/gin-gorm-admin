...
func SetLocalPathDir(s string) Option {
	return func(o *options) {
		o.LocalPathDir = s
	}
}
...
func Init(ctx context.Context, opts ...Option) (func(), error) {
	...
	if v := o.LocalPathDir; v != "" {
		config.C.LocalPath = v
	}
	...
	local := fileStore.InitLocal(config.C.LocalPath)
	injector, injectorCleanFunc, err := BuildInjector(local)
	if err != nil {
		return nil, err
	}
